...
The app uses browser state as a practical authorization boundary. `admin_session`, `auditor_id`, tier, role, and auth ID are read from `localStorage` and used for routing/data filters:
...
### Open auditor self-provisioning

Any signed-in Google user can become an auditor record if `admin_session` is present, because the app auto-inserts into `auditors`:

- `src/App.jsx:50`

That conflicts with the "authorized auditors only" product language unless open auditor signup is intentional.

### Raw Google tokens reach the browser

Member Google provider tokens are stored in `members` and then selected into the auditor browser:
...
Six-digit auditor auth IDs are generated with `Math.random()` in the client:
...
`create-subscription` receives a client-supplied `auditor_id`:

- `src/pages/MembersList.jsx:122`

The Edge Function must ignore or verify this against the authenticated user. Otherwise users can attempt payment actions for other auditors.
...
3. Add or verify Supabase RLS for `auditors`, `members`, and `audit_logs`.
4. Require server-side ownership checks in `verify_auditor_capacity`, `refresh-google-token`, and `create-subscription`.
...
3. Patch `public.verify_auditor_capacity` and `public.increment_slots` to set a fixed `search_path`, then rerun `supabase db advisors --db-url ... --type security`.
...
6. Add or verify stricter RLS policies for `public.auditors`, `public.members`, and `public.audit_logs`.
