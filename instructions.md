# Arukin Workspace Setup Instructions

Welcome to the Arukin project! Because this architecture relies on a strict separation of frontend and backend environments along with various security audits and scratch scripts, it requires a specific local directory structure. 
...
Create a directory in an appropriate work folder (e.g., `~/Work/websites/`) called `Arukin` to represent the organization. 
...
Inside the `Arukin` workspace root, clone the two main codebases. If you already have these repositories locally, simply move them into the workspace root.
...
For a deep dive into the system's architecture, security models (Token Lifecycles, CSPs), and integrations, please refer to the comprehensive **[Arukin Documentation Hub](https://github.com/arukinSec/docs)**.
