...
import { Menu, X, ArrowRight, BookOpen, CreditCard, ShieldCheck, Zap, Wrench } from 'lucide-react';
      ...
      <nav className="border-b border-slate-200 bg-white/80 backdrop-blur-md sticky top-0 z-50 shrink-0">
        ...
        {isMobileMenuOpen && (
          <div className="md:hidden bg-slate-50 border-b border-slate-200 px-6 py-4 space-y-4 shadow-2xl animate-fade-in">
            <Link to="/" className="block text-sm font-semibold text-slate-600 hover:text-emerald-600" onClick={() => setIsMobileMenuOpen(false)}>Home</Link>
            <Link to="/how-it-works" className="block text-sm font-semibold text-slate-600 hover:text-emerald-600" onClick={() => setIsMobileMenuOpen(false)}>How it works</Link>
            <Link to="/use-cases" className="block text-sm font-semibold text-slate-600 hover:text-emerald-600" onClick={() => setIsMobileMenuOpen(false)}>Use Cases</Link>
            <Link to="/about" className="block text-sm font-semibold text-slate-600 hover:text-emerald-600" onClick={() => setIsMobileMenuOpen(false)}>About</Link>
            <Link to="/pricing" className="block text-sm font-semibold text-slate-600 hover:text-emerald-600" onClick={() => setIsMobileMenuOpen(false)}>Pricing</Link>
          </div>
        )}
      </nav>
      ...
      <main className="flex-1 max-w-5xl w-full mx-auto px-6 py-16 animate-fade-in">
        <div className="text-center mb-16">
          ...
          <p className="text-slate-600 text-lg max-w-2xl mx-auto">
            Choose a category below to find detailed answers about Arukin's features, pricing, security, and open-source model.
          </p>
        </div>
